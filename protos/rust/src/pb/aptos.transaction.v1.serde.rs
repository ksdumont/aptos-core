...
impl serde::Serialize for WebAuthn {
        ...
        S: serde::Serializer,
    {
        use serde::ser::SerializeStruct;
        let mut len = 0;
        if !self.partial_authenticator_assertion_response.is_empty() {
            len += 1;
        }
        let mut struct_ser = serializer.serialize_struct("aptos.transaction.v1.WebAuthn", len)?;
        if !self.partial_authenticator_assertion_response.is_empty() {
            struct_ser.serialize_field("partialAuthenticatorAssertionResponse", pbjson::private::base64::encode(&self.partial_authenticator_assertion_response).as_str())?;
        }
        struct_ser.end()
    }
}
        ...
        impl<'de> serde::Deserialize<'de> for GeneratedField {
                ...
                D: serde::Deserializer<'de>,
            {
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                    ...
                    {
                        match value {
                            "partialAuthenticatorAssertionResponse" | "partial_authenticator_assertion_response" => Ok(GeneratedField::PartialAuthenticatorAssertionResponse),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
                }
        ...
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            ...
            fn visit_map<V>(self, mut map: V) -> std::result::Result<WebAuthn, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut partial_authenticator_assertion_response__ = None;
                while let Some(k) = map.next_key()? {
                    match k {
                        GeneratedField::PartialAuthenticatorAssertionResponse => {
                            if partial_authenticator_assertion_response__.is_some() {
                                return Err(serde::de::Error::duplicate_field("partialAuthenticatorAssertionResponse"));
                            }
                            partial_authenticator_assertion_response__ =
                                Some(map.next_value::<::pbjson::private::BytesDeserialize<_>>()?.0)
                            ;
                        }
                    }
                ...
                Ok(WebAuthn {
                    partial_authenticator_assertion_response: partial_authenticator_assertion_response__.unwrap_or_default(),
                })
            }
        }
