...
impl Constraint {
    ...
    pub fn join(&mut self, loc: &Loc, other: &Constraint) -> Result<bool, TypeUnificationError> {
...
impl Substitution {
    ...
    pub fn eval_constraint(
        ...
        if matches!(ty, Type::Error) {
            ...
            self.add_constraint(*other_var, loc.clone(), order, c)
        } else {
            match &c {
        ...
        match (t1, t2) {
            ...
            (Type::Tuple(ts1), Type::Tuple(ts2)) => {
                return Ok(Type::Tuple(self.unify_vec(
                    ...
                    // and allow e.g. reference type conversions.
            ...
            (Type::Fun(a1, r1), Type::Fun(a2, r2)) => {
                ...
                // T2 is also a T1).
                return Ok(Type::Fun(
                    Box::new(self.unify(variance, order.swap(), a1, a2)?),
                    Box::new(self.unify(variance, order, r1, r2)?),
                ));
            },
